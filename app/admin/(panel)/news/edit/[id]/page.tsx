      ...
      try {
        ...
        setFormData({
          ...
          keywords: Array.isArray(data.keywords) 
            ? data.keywords 
            : data.keywords?.split(',').filter(Boolean) || [],
        })
      } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-lg font-medium text-gray-700 dark:text-gray-300">Loading news data...</div>
      </div>
    )
  }
      ...
      <Card className="max-w-4xl mx-auto shadow-xl bg-white dark:bg-gray-800 rounded-xl">
        ...
        <CardContent className="pt-8">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {/* Video Link */}
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700 dark:text-gray-200">Video Link (Optional)</label>
              <Input 
                name="videoLink" 
